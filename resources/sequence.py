...
class TaskList(Resource):
    ...
    def get(self, set_id):
        ...
        if tasks:
            ...
            sorted_user_tasks = sorted(user_tasks, key=lambda x: x["task_position"])
            return {"set": _set.json(), "tasks": sorted_user_tasks}
...
class UserTask(Resource):
    ...
    def get(self):
        ...
        if usertask:
            return {"user_task": usertask.json()}
        else:
        ...
        if user_task:
            if data["completed"] == True:
                formatted_completion_date = datetime.date(
                    data["completion_date_year"],
                    data["completion_date_month"],
                    data["completion_date_date"],
                )
            ...
            try:
                ...
                return {"current_set_task": new_user_task.json()}
