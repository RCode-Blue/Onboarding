...
class Template(Resource):
    ...
    def put(self):
        ...
        if template:
            ...
            newTemplate = TemplateModel(data["template_name"], data["description"])
    ...
    def post(self):
        ...
        data = Template.parser.parse_args()
        new_template = TemplateModel(data["template_name"], data["description"])

        try:
            new_template.save_to_db()
            return {"newTemplate": new_template.json()}, 201
        except:
            return {"message": "An error occured inserting the template"}, 500
...
class Templates(Resource):
    def get(self):
        return {
            "templates": [template.json() for template in TemplateModel.query.all()]
        }
